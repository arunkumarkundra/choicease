...
function renderLenses() {
  ...
  body.innerHTML = `
    <p class="panel__lede">The weighted score answers "what fits my priorities best". These lenses answer different questions — worst-case protection, minimum standards, safe elimination, and structural robustness. Convergence across lenses is strong evidence; divergence is an insight in itself.</p>
...
function renderEliminator(eliminator) {
  ...
  const parts = [];
  parts.push(`<p class="lens-note">${esc(dominanceStatusLine(eliminator))}</p>`);
  if (!eliminated.length && !watchlist.length) {
    parts.push(`<p class="lens-note">Beyond that, nothing can be safely cut — every option is either competitive under some reasonable weighting or best-in-field somewhere. The shortlist is the full list.</p>`);
    return parts.join('');
  }
...
/** Dominance is Rule A of the Eliminator; its finding is always reported. */
function dominanceStatusLine(eliminator) {
  const dominated = eliminator.eliminated.filter((e) => e.rule === 'dominated');
  if (!dominated.length) {
    return 'Dominance check: no option is dominated — every option beats every other option on at least one criterion, so none can be discarded on structure alone.';
  }
  return `Dominance check: ${dominated.map((e) => `${e.option.name} is dominated by ${e.dominatedBy}`).join('; ')} — equal or worse on every criterion.`;
}
