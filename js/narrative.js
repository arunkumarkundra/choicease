...
export function executiveSummary(decision, analysis) {
  ...
  if (topDrivers.length) {
    const verb = topDrivers.length === 1 ? 'which accounts for' : 'which together account for';
    lines.push(
      `The result is driven by ${joinNames(topDrivers.map((d) => d.criterionName))}, ` +
      `${verb} ${Math.round(topDrivers.reduce((s, d) => s + d.weightPct, 0))}% of the decision weight.`,
    );
  }
