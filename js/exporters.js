  ...
  try {
    ...
    if (ranked.length > 1) {
      ...
      const elimLine = elim.eliminated.length
        ? `Eliminator: safe to cut — ${elim.eliminated.map((e) => pptxEliminationNote(e)).join('; ')}. Shortlist: ${elim.shortlist.join(', ')}.`
        : `Eliminator: nothing can be safely cut — ${dominanceStatusText(elim)}, and every option is competitive under some reasonable weighting. The shortlist is the full list.`;
      const watchLine = elim.watchlist.length
        ? `Eliminator watchlist: ${elim.watchlist.map((w) => w.option.name).join(', ')} carry deal-breakers but retain a path to #1 — cut only if the low ratings are firm.`
        : '';
      for (const line of [robustLine, regretLine, elimLine, watchLine].filter(Boolean)) {
    ...
    const method = [
      ...
      'Options rated 0–5 per criterion. Final score = sum of (rating x weight) across criteria. Unrated cells default to 2.5 (the scale midpoint) and are disclosed in the confidence assessment.',
...
export function buildPptx(PptxGenJS, analysis, summaryText) {
  ...
  if (ranked.length > 1) {
    ...
    if (eliminator.eliminated.length) {
      ...
      notes.push(`Eliminator: nothing can be safely cut — ${dominanceStatusText(eliminator)}, and every option remains competitive under some reasonable weighting.`);
    }
...
function dominanceStatusText(eliminator) {
  const dominated = eliminator.eliminated.filter((e) => e.rule === 'dominated');
  return dominated.length
    ? `${dominated.map((e) => `${e.option.name} is dominated by ${e.dominatedBy}`).join('; ')}`
    : 'no option is dominated (each beats every other on at least one criterion)';
}
