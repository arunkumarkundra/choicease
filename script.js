...
function generatePPTX() {
    ...
    slide.addText('Generated by Choicease - Smart Choices, Made Easy', {
        x: 0.5, y: 5, w: 9, h: 0.5,
    ...
    const tableData = [
        [
            ...
            { text: 'Option', options: { bold: true, fill: colors.bg } },
            { text: 'Score', options: { bold: true, fill: colors.bg } },
            { text: 'Gap from Best', options: { bold: true, fill: colors.bg } }
        ]
    ];
    ...
    advancedAnalytics.results.forEach((result, i) => {
        ...
        tableData.push([
            { text: `#${i + 1}`, options: { color: i === 0 ? colors.success : colors.text } },
            { text: result.option.name, options: { bold: i === 0 } },
            { text: result.totalScore.toFixed(2), options: { color: i === 0 ? colors.success : colors.text } },
            { text: gap > 0 ? `-${gap.toFixed(2)}` : '0.00', options: { color: gap > 0 ? colors.danger : colors.success } }
        ]);
    });
    ...
    if (advancedAnalytics.risks && advancedAnalytics.risks.length > 0) {
        ...
        advancedAnalytics.risks.slice(0, 5).forEach(risk => {
            const severityColor = {
                ...
                'low': colors.success
            }[risk.severity] || colors.textLight;
            
            slide.addShape(pptx.ShapeType.rect, {
                x: 0.5, y: yPos, w: 9, h: 0.8,
                fill: { color: risk.severity === 'critical' ? 'f8d7da' : 'fff3cd' },
...
slide.addText('Recommended Next Steps:', {
    ...
    nextSteps.forEach(step => {
        slide.addText(step, {
            x: 1, y: yPos, w: 8, h: 0.3,
            fontSize: 13, color: colors.textLight, bullet: true
        });
        yPos += 0.35;
    });
