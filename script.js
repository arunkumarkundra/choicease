...
function generatePPTX() {
    ...
    // Shortcut to SchemeColor tokens
    const S = pptx.SchemeColor;

    // Colors palette fallback (used when CHART_COLORS isn't defined in your app)
    const FALLBACK_CHART_COLORS = ['4E79A7','F28E2B','E15759','76B7B2','59A14F','EDC948','B07AA1','FF9DA7'];

    // Use the app's CHART_COLORS if present and valid; otherwise fallback
    const chartPalette = (Array.isArray(window.CHART_COLORS) && window.CHART_COLORS.length > 0)
        ? window.CHART_COLORS
        : FALLBACK_CHART_COLORS;

    // Define a slide master that uses scheme colors and placeholders
    pptx.defineSlideMaster({
        title: 'CHOICEASE_MASTER',
        background: { color: S.background1 },
        objects: [
            // header stripe (theme accent)
            { rect: { x: 0, y: 0, w: '100%', h: 0.5, fill: { color: S.accent1 } } },
            // footer stripe
            { rect: { x: 0, y: 6.9, w: '100%', h: 0.45, fill: { color: S.background2 } } },
            // Title placeholder
            { placeholder: { options: { name: 'title', type: 'title', x: 0.5, y: 0.12, w: 9, h: 0.9 } } },
            // Body placeholder
            { placeholder: { options: { name: 'body',  type: 'body',  x: 0.5, y: 1.1, w: 9, h: 5.5 } } }
        ],
        slideNumber: { x: 0.3, y: '95%', color: S.text2 }
    });

    const newSlide = () => pptx.addSlide({ masterName: 'CHOICEASE_MASTER' });

    // --- SLIDE 1: Title Slide ---
    let slide = newSlide();
    // Correct: put actual text into the title placeholder (first arg is text)
    ...
    slide.addText('📋 Decision Overview', {
        ...
        fontSize: 28, bold: true, color: S.accent1
    });

    let yPos = 0.9;
    slide.addText('Decision:', { x: 0.5, y: yPos, w: 2, h: 0.4, fontSize: 16, bold: true, color: S.text1 });
    slide.addText(decisionData.title, { x: 2.5, y: yPos, w: 6.5, h: 0.4, fontSize: 16, color: S.text2, fit: 'shrink', wrap: true });
    yPos += 0.6;

    if (decisionData.description) {
        slide.addText('Description:', { x: 0.5, y: yPos, w: 2, h: 0.4, fontSize: 16, bold: true, color: S.text1 });
        slide.addText(decisionData.description, {
            x: 2.5, y: yPos, w: 6.5, h: 1.0,
            fontSize: 14, color: S.text2, wrap: true, fit: 'shrink'
        });
        yPos += 1.1;
    }

    slide.addText(`Options (${decisionData.options.length}):`, { x: 0.5, y: yPos, w: 9, h: 0.4, fontSize: 16, bold: true, color: S.text1 });
    yPos += 0.5;

    decisionData.options.forEach((opt, i) => {
        if (yPos > 5) return; // keep content within slide bounds
        slide.addText(`${i + 1}. ${opt.name}`, {
            x: 1, y: yPos, w: 8, h: 0.3, fontSize: 14, color: S.text2, bullet: true, fit: 'shrink', wrap: true
        });
        yPos += 0.36;
    });
    ...
    slide = newSlide();
    slide.addText('📊 Executive Summary', { x: 0.5, y: 0.2, w: 9, h: 0.6, fontSize: 28, bold: true, color: S.accent1 });

    const winner = (advancedAnalytics.results && advancedAnalytics.results[0]) || { option: { name: '' }, totalScore: 0 };

    slide.addShape(pptx.ShapeType.rect, {
        x: 0.5, y: 1.1, w: 9, h: 1.2,
        fill: { color: S.background2 },
        line: { color: S.accent2, width: 2 }
    });

    slide.addText('🏆 Recommended Choice', { x: 0.7, y: 1.15, w: 8.6, h: 0.35, fontSize: 16, bold: true, color: S.accent2 });
    slide.addText(winner.option.name || '—', { x: 0.7, y: 1.5, w: 6, h: 0.7, fontSize: 22, bold: true, color: S.text1, fit: 'shrink', wrap: true });
    slide.addText(`Score: ${winner.totalScore.toFixed(2)}`, { x: 7, y: 1.5, w: 2.2, h: 0.7, fontSize: 18, color: S.accent2, align: 'right' });

    yPos = 2.8;
    slide.addText(`Confidence Level: ${advancedAnalytics.confidence.level.toUpperCase()}`, {
        x: 0.5, y: yPos, w: 9, h: 0.4, fontSize: 14, bold: true, color: S.text1
    });
    slide.addText(advancedAnalytics.confidence.explanation || '', {
        x: 0.5, y: yPos + 0.45, w: 9, h: 1.2, fontSize: 13, color: S.text2, wrap: true, fit: 'shrink'
    });

    // --- SLIDE 4: Detailed Results ---
    slide = newSlide();
    slide.addText('🎯 Detailed Results', { x: 0.5, y: 0.2, w: 9, h: 0.6, fontSize: 28, bold: true, color: S.accent1 });
    ...
    (advancedAnalytics.results || []).forEach((result, i) => {
        const gap = winner.totalScore - result.totalScore;
        resultsTable.push([
            { text: `#${i + 1}`, options: { color: i === 0 ? S.accent3 : S.text1 } },
            { text: result.option.name, options: { bold: i === 0, fontSize: 12, wrap: true } },
            { text: result.totalScore.toFixed(2), options: { color: i === 0 ? S.accent3 : S.text1 } },
            { text: gap > 0 ? `-${gap.toFixed(2)}` : '0.00', options: { color: gap > 0 ? S.accent6 : S.accent4 } }
        ]);
    });

    slide.addTable(resultsTable, {
        x: 0.5, y: 1.1, w: 9, h: 4,
        fontSize: 12,
        border: { type: 'solid', color: 'CCCCCC' },
        align: 'center',
        autoPage: true // allow the table to continue on subsequent slides if too long
    });

    // --- SLIDE 5: Criteria Weights (Pie Chart + Table) ---
    slide = newSlide();
    slide.addText('🥧 Decision Criteria Weights', { x: 0.5, y: 0.2, w: 9, h: 0.6, fontSize: 28, bold: true, color: S.accent1 });
    ...
    // Use the same palette for chart and table so they match visually
    const effectiveChartColors = chartPalette.slice(0, pieChartData[0].labels.length);
    ...
    try {
        slide.addChart(pptx.ChartType.pie, pieChartData, {
            ...
            chartColors: effectiveChartColors, // ensure pie and table use same colors
            showTitle: false
        });
    } catch (err) {
        console.warn('Pie chart creation failed:', err);
        slide.addText('Pie chart not available — see web app', { x: 0.5, y: 2.2, w: 4.2, h: 1.0, fontSize: 12, color: S.text2 });
    }

    // Right-side weights table — use same palette for bar visuals
    const weightsTable = [
        [
    ...
    (decisionData.criteria || []).forEach((criteria, i) => {
        const weight = decisionData.normalizedWeights[criteria.id] || 0;
        const barLength = Math.max(1, Math.min(8, Math.round(weight / 12.5)));
        const bar = '█'.repeat(barLength);
        // map to hex from the effectiveChartColors for visual parity with the pie
        const barHex = effectiveChartColors[i % effectiveChartColors.length] || FALLBACK_CHART_COLORS[i % FALLBACK_CHART_COLORS.length];

        weightsTable.push([
            { text: criteria.name.length > 18 ? criteria.name.substring(0, 15) + '...' : criteria.name, options: { fontSize: 11, align: 'left', wrap: true } },
            { text: `${weight.toFixed(1)}%`, options: { fontSize: 11, align: 'center', bold: true } },
            { text: bar, options: { fontSize: 10, color: barHex, align: 'left' } }
        ]);
    });
    ...
    slide = newSlide();
    slide.addText('📊 Criteria Impact Analysis', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });
    slide.addText('How each criterion affects the winner:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 12, color: S.text2 });
    ...
    (decisionData.criteria || []).forEach(criteria => {
        ...
        impactTable.push([
            { text: criteria.name, options: { fontSize: 11 } },
            { text: `${(weight * 100).toFixed(1)}%`, options: { fontSize: 11 } },
            { text: `${rating.toFixed(1)}/5`, options: { fontSize: 11 } },
            { text: contribution, options: { fontSize: 11, color: S.accent4 } }
        ]);
    });
    ...
    slide = newSlide();
    slide.addText('🔥 Performance Heatmap', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });
    ...
    (decisionData.options || []).forEach(option => {
        const row = [{ text: option.name, options: { fontSize: 11, wrap: true } }];

        (decisionData.criteria || []).forEach(criteria => {
            const ratingKey = `${option.id}-${criteria.id}`;
            const rating = (decisionData.ratings && (decisionData.ratings[ratingKey] || 0)) || 0;

            // Keep these heatmap backgrounds as hex for readability — changeable if you want scheme tokens
            ...
            row.push({ text: rating.toFixed(1), options: { fill: cellColor, fontSize: 10, align: 'center' } });
        });
    ...
    if (advancedAnalytics.sensitivity) {
        slide = newSlide();
        slide.addText('⚖️ Sensitivity Analysis', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });
        slide.addText('Decision Stability Assessment:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 14, bold: true, color: S.text1 });

        const flipPoints = (typeof computeFlipPoints === 'function') ? computeFlipPoints() : [];

        if (!flipPoints || flipPoints.length === 0) {
            slide.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.6, w: 9, h: 1.2, fill: { color: S.background2 }, line: { color: S.accent3, width: 2 } });
            slide.addText('✅ Highly Stable Decision', { x: 0.7, y: 1.8, w: 8.6, h: 0.8, fontSize: 16, bold: true, color: S.accent3, valign: 'middle' });
            slide.addText('No criteria weight changes would alter the recommended choice', { x: 0.5, y: 3.0, w: 9, h: 0.5, fontSize: 12, color: S.text2, align: 'center' });
        } else {
            ...
            slide.addTable(sensitivityTable, { x: 0.5, y: 1.6, w: 9, h: 3.5, fontSize: 12, border: { type: 'solid', color: 'CCCCCC' }, autoPage: true });
        }
    }

    // --- SLIDE 9: Satisficers Analysis ---
    slide = newSlide();
    slide.addText('✅ Satisficers Analysis', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });
    slide.addText('Options that meet minimum acceptable standards:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 13, color: S.text2 });
    ...
    if (satisficers.length > 0) {
        satisficers.forEach((result, i) => {
            slide.addShape(pptx.ShapeType.rect, {
                ...
                fill: { color: i === 0 ? S.background2 : S.background1 },
                line: { color: i === 0 ? S.accent3 : S.text2, width: 1 }
            });

            slide.addText(`${i + 1}. ${result.option.name}`, { x: 0.7, y: yPos + 0.05, w: 6, h: 0.5, fontSize: 13, bold: i === 0, color: S.text1, fit: 'shrink', wrap: true });
            slide.addText(`Score: ${result.totalScore.toFixed(2)}`, { x: 7, y: yPos + 0.05, w: 2.2, h: 0.5, fontSize: 13, align: 'right', color: S.accent4 });
    ...
    if (advancedAnalytics.risks && advancedAnalytics.risks.length > 0) {
        slide = newSlide();
        slide.addText('⚠️ Risk Analysis', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });

        yPos = 1.2;
        (advancedAnalytics.risks || []).slice(0, 5).forEach(risk => {
            const severityColor = { 'critical': S.accent6, 'high': S.accent5, 'moderate': S.accent2, 'low': S.accent4 }[risk.severity] || S.text2;
            const fillColor = { 'critical': 'f8d7da', 'high': 'fff3cd', 'moderate': 'fff3cd', 'low': 'd4edda' }[risk.severity] || 'ffffff';

            slide.addShape(pptx.ShapeType.rect, { x: 0.5, y: yPos, w: 9, h: 0.8, fill: { color: fillColor }, line: { color: severityColor, width: 1 } });
            slide.addText(`${risk.severity.toUpperCase()}: ${risk.description}`, { x: 0.7, y: yPos + 0.1, w: 8.6, h: 0.6, fontSize: 12, color: S.text1, wrap: true, fit: 'shrink' });
    ...
    slide = newSlide();
    slide.addText('✅ Next Steps & Recommendations', { x: 0.5, y: 0.2, fontSize: 26, bold: true, color: S.accent1 });

    slide.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.2, w: 9, h: 1.5, fill: { color: S.background2 }, line: { color: S.accent4, width: 3 } });
    slide.addText(`Based on your criteria, we recommend: ${winner.option.name}`, { x: 0.7, y: 1.4, w: 8.6, h: 1.1, fontSize: 20, bold: true, color: S.text1, valign: 'middle', fit: 'shrink', wrap: true });

    if (winner.option.description) {
        slide.addText(winner.option.description, { x: 0.5, y: 2.9, w: 9, h: 0.6, fontSize: 13, color: S.text2, wrap: true, fit: 'shrink' });
        yPos = 3.6;
    } else {
        yPos = 2.9;
    }

    slide.addText('Recommended Next Steps:', { x: 0.5, y: yPos, w: 9, h: 0.4, fontSize: 16, bold: true, color: S.text1 });
    ...
    nextSteps.forEach(step => {
        slide.addText(step, { x: 1, y: yPos, w: 8, h: 0.3, fontSize: 12, color: S.text2, bullet: true, fit: 'shrink', wrap: true });
        yPos += 0.34;
    });

    // Save the presentation
    const fileName = `choicease_advanced_${(decisionData.title || 'report').replace(/[^a-z0-9]/gi, '_')}_${Date.now()}.pptx`;
