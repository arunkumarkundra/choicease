...
function generatePPTX() {
    ...
    pptx.defineSlideMaster({
        ...
        objects: [
            ...
            // Title placeholder (native placeholder -> theme-aware)
            {
                placeholder: {
                    options: { name: 'title', type: 'title', x: 0.5, y: 0.12, w: 9, h: 0.9 }
                }
            },
            // Body placeholder
            {
                placeholder: {
                    options: { name: 'body', type: 'body', x: 0.5, y: 1.1, w: 9, h: 5.5 }
                }
            }
        ],
    ...
    // Helper to create a new slide using master
    const newSlide = () => pptx.addSlide({ masterName: 'CHOICEASE_MASTER' });

    // --- SLIDE 1: Title Slide ---
    let slide = newSlide();
    // Put the main title in the title placeholder (PowerPoint will map fonts/colors)
    slide.addText('Advanced Decision Analysis', {
        ...
        options: {
            ...
            align: 'center',
            valign: 'middle',
            color: S.background1 // this will contrast with the accent header stripe
        }
    });

    // Subtitle (theme-aware text color)
    slide.addText(decisionData.title, {
        x: 0.5, y: 2.3, w: 9, h: 0.8,
        fontSize: 28, color: S.text1, align: 'center'
    });

    slide.addText('Generated by Choicease.com - Smart Choices, Made Easy', {
        x: 0.5, y: 4.9, w: 9, h: 0.5,
        fontSize: 14, color: S.text2, align: 'center', italic: true
    });
    ...
    slide.addText('📋 Decision Overview', {
        ...
        fontSize: 32, bold: true, color: S.accent1
    });

    let yPos = 0.9;
    slide.addText('Decision:', {
        x: 0.5, y: yPos, w: 2, h: 0.4,
        fontSize: 16, bold: true, color: S.text1
    });
    slide.addText(decisionData.title, {
    ...
    if (decisionData.description) {
        ...
        slide.addText(decisionData.description, {
            x: 2.5, y: yPos, w: 7, h: 1,
            fontSize: 14, color: S.text2
        });
        yPos += 1.2;
    }

    slide.addText(`Options (${decisionData.options.length}):`, {
    ...
    decisionData.options.forEach((opt, i) => {
        ...
        slide.addText(`${i + 1}. ${opt.name}`, {
            ...
            fontSize: 14, color: S.text2, bullet: true
        });
        yPos += 0.4;
    });
    ...
    slide.addShape(pptx.ShapeType.rect, {
        ...
        // Use scheme color for fill so theme can adjust it; use accent2 for emphasis
        fill: { color: S.background2 },
        line: { color: S.accent2, width: 2 }
    });

    slide.addText('🏆 Recommended Choice', {
        x: 0.7, y: 1.15, w: 8.6, h: 0.35,
        fontSize: 18, bold: true, color: S.accent2
    });

    slide.addText(winner.option.name, {
        x: 0.7, y: 1.6, w: 6, h: 0.6,
        fontSize: 24, bold: true, color: S.text1
    });

    slide.addText(`Score: ${winner.totalScore.toFixed(2)}`, {
        x: 7, y: 1.6, w: 2.2, h: 0.6,
        fontSize: 20, color: S.accent2, align: 'right'
    });
    ...
    slide.addText(advancedAnalytics.confidence.explanation, {
        x: 0.5, y: yPos + 0.5, w: 9, h: 1.2,
        fontSize: 14, color: S.text2
    });

    // --- SLIDE 4: Detailed Results ---
    slide = newSlide();
    slide.addText('🎯 Detailed Results', {
        x: 0.5, y: 0.2, w: 9, h: 0.6,
        fontSize: 32, bold: true, color: S.accent1
    });
    ...
    advancedAnalytics.results.forEach((result, i) => {
        ...
        tableData.push([
            ...
            { text: result.option.name, options: { bold: i === 0 } },
    ...
    try {
        ...
        // Omit chartColors to allow PowerPoint theme to apply default palette
        ...
        // Fallback visual (theme-aware)
        slide.addShape(pptx.ShapeType.rect, {
            x: 0.5, y: 1.0, w: 4.2, h: 3.8,
            fill: { color: S.background2 },
            line: { color: S.text2, width: 1 }
        });

        slide.addText('📊 Pie Chart\n(Multiple criteria view\navailable in web app)', {
            x: 0.5, y: 2.8, w: 4.2, h: 1.2,
            fontSize: 14, color: S.text2, align: 'center', valign: 'middle'
        });
    }

    // Right-side weights table
    const weightsTable = [
        [
            { text: 'Criteria', options: { bold: true, fill: S.background2, fontSize: 13, align: 'center' } },
            { text: 'Weight (%)', options: { bold: true, fill: S.background2, fontSize: 13, align: 'center' } },
            { text: 'Bar', options: { bold: true, fill: S.background2, fontSize: 13, align: 'center' } }
        ]
    ];
    ...
    decisionData.criteria.forEach((criteria, i) => {
        ...
        // Bar visual: use block characters; keep color theme-aware by picking scheme accents for emphasis
        const barLength = Math.max(1, Math.min(8, Math.round(weight / 12.5)));
        const bar = '█'.repeat(barLength);
        const barColor = [S.accent1, S.accent2, S.accent3, S.accent4, S.accent5, S.accent6][i % 6] || S.accent1;

        weightsTable.push([
            { text: criteria.name.length > 18 ? criteria.name.substring(0, 15) + '...' : criteria.name, options: { fontSize: 11, align: 'left' } },
            { text: `${weight.toFixed(1)}%`, options: { fontSize: 11, align: 'center', bold: true } },
            { text: bar, options: { fontSize: 10, color: barColor, align: 'left' } }
        ]);
    });
    ...
    slide = newSlide();
    slide.addText('📊 Criteria Impact Analysis', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });
    slide.addText('How each criterion affects the winner:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 14, color: S.text2 });
    ...
    slide = newSlide();
    slide.addText('🔥 Performance Heatmap', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });
    ...
    decisionData.options.forEach(option => {
        ...
        decisionData.criteria.forEach(criteria => {
            ...
            // Heatmap cell color: these are intentionally fixed colors for readability
            // You can replace these with scheme colors if you want theme mapping.
    ...
    if (advancedAnalytics.sensitivity) {
        slide = newSlide();
        slide.addText('⚖️ Sensitivity Analysis', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });
        slide.addText('Decision Stability Assessment:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 16, bold: true, color: S.text1 });
        ...
        if (!flipPoints || flipPoints.length === 0) {
            ...
            slide.addText('✅ Highly Stable Decision', {
                ...
                fontSize: 18, bold: true, color: S.accent3, valign: 'middle'
            });

            slide.addText('No criteria weight changes would alter the recommended choice', {
                x: 0.5, y: 3.0, w: 9, h: 0.5,
                fontSize: 14, color: S.text2, align: 'center'
            });
        } else {
    ...
    slide = newSlide();
    slide.addText('✅ Satisficers Analysis', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });
    slide.addText('Options that meet minimum acceptable standards:', { x: 0.5, y: 1.0, w: 9, h: 0.4, fontSize: 14, color: S.text2 });
    ...
    if (satisficers.length > 0) {
        ...
        slide.addText('No options meet the minimum threshold', {
            ...
            fontSize: 16, color: S.accent6, align: 'center'
        });
    }
    ...
    if (advancedAnalytics.risks && advancedAnalytics.risks.length > 0) {
        slide = newSlide();
        slide.addText('⚠️ Risk Analysis', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });

        yPos = 1.2;
        advancedAnalytics.risks.slice(0, 5).forEach(risk => {
            // Map severity to scheme accents where sensible (fallback to explicit hex for background)
            const severityColor = {
                'critical': S.accent6,
                'high': S.accent5,
                'moderate': S.accent2,
                'low': S.accent4
            }[risk.severity] || S.text2;

            // Use a subtle fill (some fills kept as light hexs for clarity)
            const fillColor = {
                'critical': 'f8d7da',
                'high': 'fff3cd',
                'moderate': 'fff3cd',
                'low': 'd4edda'
            }[risk.severity] || 'ffffff';

            slide.addShape(pptx.ShapeType.rect, {
                x: 0.5, y: yPos, w: 9, h: 0.8,
                fill: { color: fillColor },
                line: { color: severityColor, width: 1 }
            });

            slide.addText(`${risk.severity.toUpperCase()}: ${risk.description}`, {
                x: 0.7, y: yPos + 0.1, w: 8.6, h: 0.6,
                fontSize: 12, color: S.text1
            });
    ...
    slide = newSlide();
    slide.addText('✅ Next Steps & Recommendations', { x: 0.5, y: 0.2, fontSize: 32, bold: true, color: S.accent1 });

    slide.addShape(pptx.ShapeType.rect, {
        x: 0.5, y: 1.2, w: 9, h: 1.5,
        fill: { color: S.background2 },
        line: { color: S.accent4, width: 3 }
    });

    slide.addText(`Based on your criteria, we recommend: ${winner.option.name}`, {
        x: 0.7, y: 1.4, w: 8.6, h: 1.1,
        fontSize: 22, bold: true, color: S.text1, valign: 'middle'
    });
    ...
    nextSteps.forEach(step => {
        slide.addText(step, {
            x: 1, y: yPos, w: 8, h: 0.3,
            fontSize: 13, color: S.text2, bullet: true
        });
        yPos += 0.35;
    });

    // Finalize filename & write
    const fileName = `choicease_advanced_${decisionData.title.replace(/[^a-z0-9]/gi, '_')}_${Date.now()}.pptx`;

    pptx.writeFile({ fileName: fileName })
        .then(() => {
            showToast('PowerPoint presentation generated successfully!', 'success');
        })
        .catch(err => {
