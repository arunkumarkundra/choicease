...
function generatePPTX() {
    ...
    const tableData = [
        [
            { text: 'Rank', options: { bold: true, fill: UI.bgLight } },
            { text: 'Option', options: { bold: true, fill: UI.bgLight } },
            { text: 'Score', options: { bold: true, fill: UI.bgLight } },
            { text: 'Gap from Best', options: { bold: true, fill: UI.bgLight } }
        ]
    ];
    ...
    slide.addTable(tableData, {
        x: 0.5, y: 1.05, w: 9, h: 4.8,
        fontSize: 12, border: { type: 'solid', color: UI.tableBorder },
        autoPage: true
    });
    ...
    const impactRows = [
        [
            ...
            { text: 'Winner Rating', options: { bold: true, fill: UI.bgLight } },
            { text: 'Contribution', options: { bold: true, fill: UI.bgLight } }
        ]
    ];
    ...
    slide.addTable(heatRows, { x: 0.3, y: 1.05, w: 9.4, h: 4.6, fontSize: 10, border: { type: 'solid', color: UI.tableBorder }, autoPage: true });
    ...
    if (advancedAnalytics.sensitivity) {
        ...
        if (!flipPoints || flipPoints.length === 0) {
            slide.addShape(pptx.ShapeType.rect, { x: 0.5, y: 1.5, w: 9, h: 1.2, fill: { color: UI.bgLight }, line: { color: UI.primary, width: 2 } });
            slide.addText('✅ Highly Stable Decision', { x: 0.7, y: 1.65, w: 8.6, h: 0.6, fontSize: 16, bold: true, color: UI.primary });
            slide.addText('No criteria weight changes would alter the recommended choice', { x: 0.5, y: 2.9, w: 9, h: 0.5, fontSize: 12, color: UI.textLight, align: 'center' });
        } else {
            const sensRows = [
                [
                    { text: 'Criteria', options: { bold: true, fill: UI.bgLight } },
                    { text: 'Current Weight', options: { bold: true, fill: UI.bgLight } },
                    { text: 'Flip Point', options: { bold: true, fill: UI.bgLight } },
                    { text: 'Buffer', options: { bold: true, fill: UI.bgLight } }
                ]
            ];
            ...
            slide.addTable(sensRows, { x: 0.5, y: 1.5, w: 9, h: 3.5, fontSize: 11, border: { type: 'solid', color: UI.tableBorder }, autoPage: true });
        }
    }
    ...
    if (satisficers.length > 0) {
        ...
        slide.addText('No options meet the minimum threshold', { x: 0.5, y: 2.5, w: 9, h: 0.5, fontSize: 14, color: UI.danger, align: 'center' });
    }
